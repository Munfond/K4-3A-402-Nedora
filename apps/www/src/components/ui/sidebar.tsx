...
function SidebarProvider({
  ...
  // Sync with localStorage only once on mount (after hydration is complete)
  // Empty dependency array ensures this runs exactly once after the component mounts,
  ...
  React.useEffect(() => {
    try {
      ...
      if (stored !== null) {
        ...
        // Only update if different from the initial default value
        if (storedState !== defaultOpen) {
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // Only save localStorage state for desktop (not mobile)
...
function SidebarInput({
  ...
  return (
    <Input
      ...
      data-sidebar="input"
      className={cn(
        "mt-8 h-8 w-full rounded-full bg-background shadow-none",
        className,
      )}
      {...props}
    />

...
| **P0c** ✅ | Thay `analysis-progress-panel` bằng `components/studio/pipeline-flow.tsx`: vẽ 5 node người dùng cố định (`PIPELINE_GRAPH`), cập nhật trực tiếp qua SSE, tự thu gọn thành 1 dòng khi hoàn tất. | Đã hoàn tất ở R0.6 |
| **P1** ✅ | Tách `lap-ho-so` chạy trước `lap-phuong-an`, phát sự kiện `partial: cases.ready` sớm trước khi bất kỳ worker `de-xuat` nào bắt đầu; UI nhận và hiển thị các thẻ vùng ngay lập tức. | Đã hoàn tất ở R0.3 |
| **P2** ✅ | Vòng lặp song song ≤ 3 vùng; kiểm tra patch từng vùng với vòng sửa theo lỗi ≤ 2 (`node.retry`); phát `partial: case.options.ready` khi từng vùng xong; lỗi một vùng không làm hỏng toàn bộ đợt phân tích. | Đã hoàn tất ở R0.4 |
